...
export default function AdminPage() {
  ...
  const [selectedDoctor, setSelectedDoctor] = useState<Doctor | DoctorApplication | null>(null);
  ...
  const handleDoctorApproval = async (doctorId: string, action: 'approve' | 'reject') => {
    try {
      ...
      console.error(`Error ${action}ing doctor application:`, error);
      ...
      <div className="border-b">
        <nav className="flex space-x-8">
          {[
            ...
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id as 'overview' | 'doctors' | 'users' | 'analytics')}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg max-w-md w-full mx-4 border border-black/20">
            ...
            <div className="space-y-2">
              ...
              <p><strong>Phone:</strong> {'doctor_profile' in selectedDoctor ? (selectedDoctor as Doctor).doctor_profile?.phone : (selectedDoctor as DoctorApplication).phone || 'N/A'}</p>
              <p><strong>Specialization:</strong> {'doctor_profile' in selectedDoctor ? (selectedDoctor as Doctor).doctor_profile?.specialization?.join(', ') : (selectedDoctor as DoctorApplication).specialization?.join(', ') || 'N/A'}</p>
              <p><strong>Bio:</strong> {'doctor_profile' in selectedDoctor ? (selectedDoctor as Doctor).doctor_profile?.bio : (selectedDoctor as DoctorApplication).bio || 'N/A'}</p>
              <p><strong>Status:</strong> {'status' in selectedDoctor ? (selectedDoctor as DoctorApplication).status : 'Active'}</p>
              <p><strong>Applied At:</strong> {'applied_at' in selectedDoctor ? formatDate(new Date((selectedDoctor as DoctorApplication).applied_at)) : 'N/A'}</p>
            </div>
